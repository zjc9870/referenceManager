...
public class MassageController {
    ...
    @RequestMapping("/getToUserList")
    @ResponseBody
    public Map<String,Object> getToUserList(String[] friendIdList){
        List<UserVo> toUserList = new ArrayList<UserVo>();
        for(int i=0;i<friendIdList.length;i++){
            toUserList.add(userService.getUserById(friendIdList[i]));
        }
        Map<String,Object> map = new HashMap<String, Object>();
        map.put("toUserList",toUserList);
        return map;
    }
    ...
    @ResponseBody
    public Map<String,Object> getSelectFriendList(String[] selectedIdList){
        UserVo user = userService.getLoginUser();
